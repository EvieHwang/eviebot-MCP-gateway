"""OAuth 2.1 authentication for the MCP Gateway.

Reuses the CognitoAuthProvider pattern from fastmail-mcp-server.
Proxies Cognito OIDC metadata as OAuth AS metadata, provides a DCR
endpoint, and validates JWTs via Cognito JWKS.
"""
...
async def _oauth_register(request: Request) -> JSONResponse:
    ...
    Returns pre-registered Cognito App Client credentials.
    """
...
class SlashNormalizationMiddleware:
    ...
    authorization_servers becomes "https://host/". Clients then construct
    "https://host//.well-known/..." which 404s without this middleware.
    """
...
def _protected_resource_metadata_json(base_url: str, mcp_path: str) -> dict:
    """Build protected resource metadata dict with clean URLs."""
    return {
...
class GatewayAuthProvider(RemoteAuthProvider):
    """RemoteAuthProvider adapted for Cognito + claude.ai.

    Same pattern as CognitoAuthProvider in fastmail-mcp-server:
    1. Serves /.well-known/oauth-protected-resource at root
    2. Proxies Cognito OIDC metadata as OAuth AS metadata (RFC 8414)
    3. Provides DCR endpoint returning pre-registered Cognito client credentials
    4. Normalizes double-slash paths from AnyHttpUrl trailing slashes
    """
...
def create_auth_provider() -> GatewayAuthProvider:
